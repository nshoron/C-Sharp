...
namespace homepage
{
    public partial class Manage_Customer : Form
    {
        ...
        private void LoadDataIntoDataGridView()
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT username,fullname,gender,email,mobileNumber,picture FROM REGISTRATION where ROLE='CUSTOMER'"; // Replace YourTableName with your actual table name
        ...
        private void DisplayInformationFromCurrentRow()
        {
            try
            {
                if (dataGridView1.Rows.Count > 0 && dataGridView1.Rows[currentRowIndex].Cells.Count >= 5)
                {
